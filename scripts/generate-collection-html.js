...
// 本地自动化版本：直接从已同步的 H5 stories.json 读取，输出 bedtime-story-collection.html
// （与仓库 CI 版本不同：仓库版本从 repo 根读取并写 collection.html；本文件供本地自动化调用）
const H5_DIR = 'C:/Users/Administrator/WorkBuddy/Claw/bedtime-story-app';
const STORIES_PATH = path.join(process.cwd(), 'stories.json');
const OUTPUT_PATH = path.join(process.cwd(), 'bedtime-story-collection.html');
